...
class Submission_Handler {
	...
	protected function is_valid_submission() {
		...
		} elseif ( $this->is_unsubscribed( $this->get_submitted_address(), $this->get_submitted_list() ) ) {
			$this->errors[] = 'unsubscribed';
		} elseif ( $this->is_already_subscribed( $this->get_submitted_address(), $this->get_submitted_list() ) ) {
	...
	protected function is_already_subscribed( $address, $list ) {
		$api = Plugin::instance()->api();
		$member = $api->get( 'lists/'.$list.'/members/'.$address );
		if ( $member['response']['code'] == 200 ) {
			$this->error_details['already-subscribed'][] = $list;
		}
		return !empty($this->error_details['already-subscribed']);
	}

	protected function is_unsubscribed( $address, $list ) {
		$api = Plugin::instance()->api();
		$response = $api->get( 'lists/'.$list.'/members/'.$address );
		if ( $response['response']['code'] == 200 && $response['body']->member->subscribed === false ) {
			$this->error_details['unsubscribed'][] = $list;
		}
	...
	protected function save_subscription_request() {
		...
		$confirmation->set_list($this->get_submitted_list());
		$confirmation->save();
	...
	protected function get_submitted_list() {
		$list = isset($this->submission['mailgun-list']) ? $this->submission['mailgun-list'] : '';
		return $list;
	}
	...
	protected function get_confirmation_email_subject() {
		return apply_filters( 'mailgun_confirmation_email_subject', sprintf( '[%s] Confirm Your Subscription', get_bloginfo('name') ) );
	}
	...
	protected function get_confirmation_url( $confirmation_id ) {
		...
		$url = get_site_url() ."/subscription-confirmation";
	...
	protected function get_formatted_list() {
		$requested_list = $this->get_submitted_list();
